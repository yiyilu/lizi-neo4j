...
<script>
  ...
  export default {
    ...
    methods:{
      getOutNeighbors() {
        this.loading = true;
        getOutNeighbors(this.from).then(response => {
      ...
      myEcharts() {
        ...
        this.graph.nodes.forEach(function (node) {
          /*将node中的personWebPic 拼接成地址，然后放到symbol中*/
          /*console.log(node.personWebPic)
          let str = node.personWebPic
          let len =str.length
          str = str.slice(0,len-4)
          console.log(str)
          let picAddr='image://'+require('/base/nosql/'+str+'.jpg')
          symbol.push(picAddr)*/
          node.label = {
            show: true
          };
...
*/
      ...
      async getMap() {
        this.loading = true;
        await getPersonWebMapFrom(this.from).then(response => {
          this.graph = {
            nodes: [],
            links: []
          };
